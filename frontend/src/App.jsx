...
function App() {
  ...
  const addGarment = async (key) => {
    ...
    try {
      ...
      setGarmentJobs(prev => [...prev, {
        ...
        pricing: {
          base: GARMENT_PRICES[template.name] || 15000, fabric: 0,
  ...
  const serialiseWizard = () => ({
    ...
    prices: quotePrices,
    delivery: {
      method: deliveryMethod, courier: courierService,
  ...
  const hydrateWizard = async (draft) => {
    ...
    if (prices) setQuotePrices({
      packaging: prices.packaging ?? 500,
          ...
          <aside className={`portal-sidebar ${mobileNavOpen ? 'mobile-open' : ''}`}>
            ...
            <nav className="portal-menu">
              {(!currentUser.role || currentUser.role === 'Owner') ? (
                <>
                  <div className="portal-menu-group">{t('nav.groupDaily', 'Daily')}</div>
                  ...
                  <div className="portal-menu-group">{t('nav.groupStock', 'Stock')}</div>
                  <a className={`portal-menu-item ${dashboardTab === 'fabrics' ? 'active' : ''}`} onClick={() => { setDashboardTab('fabrics'); setSelectedDirectoryCustomer(null); setMobileNavOpen(false); }}><Compass size={16} /> {t('nav.manageFabrics', 'Manage Fabrics')}</a>
                  <a className={`portal-menu-item ${dashboardTab === 'inventory' ? 'active' : ''}`} onClick={() => { setDashboardTab('inventory'); setSelectedDirectoryCustomer(null); setMobileNavOpen(false); }}><Package size={16} /> {t('nav.inventory', 'Inventory')}</a>

                  <div className="portal-menu-group">{t('nav.groupPeople', 'People')}</div>
                  <a className={`portal-menu-item ${dashboardTab === 'tailors' ? 'active' : ''}`} onClick={() => { setDashboardTab('tailors'); setSelectedDirectoryCustomer(null); setMobileNavOpen(false); }}><Scissors size={16} /> {t('nav.manageTailors', 'Manage Tailors')}</a>
                  ...
                  <div className="portal-menu-group">{t('nav.groupBusiness', 'Business')}</div>
                  <a className={`portal-menu-item ${dashboardTab === 'invoices' ? 'active' : ''}`} onClick={() => { setDashboardTab('invoices'); setSelectedDirectoryCustomer(null); setMobileNavOpen(false); }}><FileText size={16} /> {t('nav.invoices', 'Invoices')}</a>
                  <a className={`portal-menu-item ${dashboardTab === 'analytics' ? 'active' : ''}`} onClick={() => { setDashboardTab('analytics'); setSelectedDirectoryCustomer(null); setMobileNavOpen(false); }}><BarChart2 size={16} /> {t('nav.analytics', 'Analytics')}</a>
                </>
                ...
                <>
                  <a className={`portal-menu-item ${dashboardTab === 'designWork' ? 'active' : ''}`} onClick={() => { setDashboardTab('designWork'); setSelectedDirectoryCustomer(null); setMobileNavOpen(false); }}><PenTool size={16} /> {t('nav.myWork', 'My Work')}</a>
                  <a className={`portal-menu-item ${dashboardTab === 'designs' ? 'active' : ''}`} onClick={() => { setDashboardTab('designs'); setSelectedDirectoryCustomer(null); setMobileNavOpen(false); }}><Sparkles size={16} /> {t('nav.designStudio', 'Design Studio')}</a>
                </>
              ) : (
                <>
                  <a className={`portal-menu-item ${dashboardTab === 'assignments' ? 'active' : ''}`} onClick={() => { setDashboardTab('assignments'); setSelectedDirectoryCustomer(null); setMobileNavOpen(false); }}><Scissors size={16} /> {t('nav.myAssignments', 'My Assignments')}</a>
                  <a className={`portal-menu-item ${dashboardTab === 'staff' ? 'active' : ''}`} onClick={() => { setDashboardTab('staff'); setSelectedDirectoryCustomer(null); setMobileNavOpen(false); }}><Clock size={16} /> {t('nav.myAttendance', 'My Attendance')}</a>
                </>
              )}
              <div className="portal-menu-divider" />
              <a className={`portal-menu-item ${dashboardTab === 'account' ? 'active' : ''}`} onClick={() => { setDashboardTab('account'); setSelectedDirectoryCustomer(null); setMobileNavOpen(false); }}><User size={16} /> {t('nav.account', 'My Account')}</a>
              <a className={`portal-menu-item ${dashboardTab === 'settings' ? 'active' : ''}`} onClick={() => { setDashboardTab('settings'); setSelectedDirectoryCustomer(null); setMobileNavOpen(false); }}><Settings size={16} /> {t('nav.settings', 'Settings')}</a>
              <a className="portal-menu-item" onClick={() => { setShowLogoutConfirm(true); setMobileNavOpen(false); }}><LogOut size={16} /> {t('nav.logout', 'Logout')}</a>
            </nav>
          ...
          <main className="portal-main">
                ...
                <div className="design-manager-content" style={{ marginTop: '24px' }}>
                  ...
                  <div className="tabs-header" style={{ marginBottom: '16px' }}>
                    ...
                    <button className={`tab-btn ${designsView === 'library' ? 'active' : ''}`}
                      ...
                      {t('designsPage.boutiqueDesigns', 'Boutique Designs')}
                    </button>
                  </div>
            ...
            {dashboardTab === 'invoices' && (
              <>
                ...
                <div className="invoices-content" style={{ marginTop: '24px', borderRadius: '14px', border: '1px solid var(--border-color, #e2e8f0)', boxShadow: '0 4px 12px rgba(0, 0, 0, 0.03)', background: 'var(--surface-color, #ffffff)', width: '100%', overflow: 'hidden' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', tableLayout: 'auto' }}>
                    <thead>
                      <tr style={{ borderBottom: '1px solid var(--border-color, #e2e8f0)', background: 'var(--bg-secondary, #fafbfc)' }}>
                        <th style={{ padding: '12px 10px', fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px', color: 'var(--text-secondary, #64748b)' }}>{t('invoicesPage.invoiceId', 'Invoice ID')}</th>
                        <th style={{ padding: '12px 10px', fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px', color: 'var(--text-secondary, #64748b)' }}>{t('invoicesPage.billingClient', 'Billing Client')}</th>
                        <th style={{ padding: '12px 10px', fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px', color: 'var(--text-secondary, #64748b)' }}>{t('common.date', 'Date')}</th>
                        <th style={{ padding: '12px 10px', fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px', color: 'var(--text-secondary, #64748b)' }}>{t('invoicesPage.totalPrice', 'Total Price')}</th>
                        <th style={{ padding: '12px 10px', fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px', color: 'var(--text-secondary, #64748b)' }}>{t('invoicesPage.totalPaid', 'Total Paid')}</th>
                        <th style={{ padding: '12px 10px', fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px', color: 'var(--text-secondary, #64748b)' }}>{t('invoicesPage.balanceDue', 'Balance Due')}</th>
                        <th style={{ padding: '12px 10px', fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px', color: 'var(--text-secondary, #64748b)' }}>{t('common.status', 'Payment Status')}</th>
                        <th style={{ padding: '12px 10px', fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px', color: 'var(--text-secondary, #64748b)' }}>{t('common.actions', 'Action')}</th>
                      </tr>
                    </thead>
                    <tbody>
                      {(() => {
                        ...
                        if (filtered.length === 0) {
                          return (
                            <tr>
                              <td colSpan="8" style={{ padding: '24px 16px', textAlign: 'center', color: 'var(--text-muted)' }}>
                                {ordersList.length === 0
                        ...
                        return filtered.map(order => {
                          const balanceDue = Math.max(0, Number(order.total_amount) - Number(order.amount_paid || 0));
                          ...
                          return (
                            <tr key={order.id} style={{ borderBottom: '1px solid var(--border-color, #f1f5f9)', fontSize: '13px', transition: 'background-color 0.15s ease' }}>
                              ...
                              <td style={{ padding: '12px 10px', fontWeight: 600, color: 'var(--text-primary, #0f172a)' }}>
                                {order.customer_name}
                              </td>
                              <td style={{ padding: '12px 10px', color: 'var(--text-secondary, #64748b)', fontSize: '12px' }}>
                              ...
                              <td style={{ padding: '12px 10px' }}>
                                ...
                                >
                                  <option value="Pending" style={{ background: '#fff', color: '#0f172a' }}>{t('invoicesPage.pending', 'Pending')}</option>
                                  {order.payment_status === 'Partially Paid' && (
                                    <option value="Partially Paid" style={{ background: '#fff', color: '#0f172a' }}>{t('invoicesPage.partiallyPaid', 'Partially Paid')}</option>
                                  )}
                                  <option value="Paid" style={{ background: '#fff', color: '#0f172a' }}>{t('invoicesPage.paid', 'Paid')}</option>
                                </select>
                              </td>
                              <td style={{ padding: '12px 10px' }}>
                                <div style={{ display: 'flex', gap: '6px', alignItems: 'center', flexWrap: 'wrap' }}>
                                  {balanceDue > 0 && (
                                    ...
                                    >
                                      <Plus size={12} /> {t('invoicesPage.addPayment', 'Add Payment')}
                                    </button>
                                  )}
                                  <button
                                    type="button"
